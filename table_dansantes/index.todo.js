...
/*function drawTable() {
  ...
  // on récupere les données saisit dans  table et modulo
  getData();  
  // nettoie le canvas
  ctx.clearRect(0,0,canvas.width,canvas.height);
  // dessine le cercle
  drawBackground();
  // i  c'est l'élément   courant et step correspond l'angle 

  // le pas du cercle    c'est l'angles qu'on utilse pour fragmenter le cercle 
  let step = 2 *Math.PI/modulo;
  
  
for (i=0; i<=modulo;++)
  {  
    let x= w/2+radius*Math.cos(i*step -Math.PI/2); 
    let y=h/2+radius*Math.cos(i*step -Math.PI/2);
  
    }

var color=(i===0)="rgb (255,255,0)";
drawPoint(x1,y1,color);
var calcul =i*table %modulo;
let x2= w/2+radius*Math.cos(i*step -Math.PI/2);
let y2=h/2+radius*Math.cos(i*step -Math.PI/2);
//
